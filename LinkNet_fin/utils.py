...
def divide_complex(dataset = 'two_stage_train',dst = 'two_stage_dataset',crop_num = 100,val_p = 0.1):
    ...
    train_label_dir = sorted(train_label_dir)
    print(train_dir)
    print(train_label_dir)
    for i in range(len(train_dir)):
        train_img = torch.FloatTensor(get_img_multi(os.path.join(train_path,train_dir[i])))
        train_label = torch.FloatTensor(get_img(os.path.join(train_label_path,train_label_dir[i+13])))              #suit mac FS
        print('begin random crop for {}th img'.format(i))
        print(train_dir[i])
        print(train_label_dir[i+13])
    ...
    for i in range(len(train_img)):
        for s in range(crop_num):
            x = np.random.randint(8935)
            y = np.random.randint(8934)
            img = train_img[i][x:x+1024,y:y+1024].numpy()
            label = train_label[i][x:x+1024,y:y+1024].numpy()
            
            num = i*crop_num + s
            if np.random.rand() > val_p:
                print(out_train_path)
                cv.imencode('.png',img)[1].tofile(os.path.join(out_train_path,'_{}.png').format(num))
                cv.imencode('.png',label)[1].tofile(os.path.join(out_train_label_path,'_{}.png').format(num))
            else:
                cv.imencode('.png',img)[1].tofile(os.path.join(out_val_path,'_{}.png').format(num))
                cv.imencode('.png',label)[1].tofile(os.path.join(out_val_label_path,'_{}.png').format(num))
    '''
#divide_complex()

def random_erasing(img,label, num_circle = 3 , rl = 25,rh = 75,
                       num_rec = 8 , xl = 25,xh = 75, yl = 25,  yh = 75):
...
def erasing(dataset,sum = 2000):
    ...
    for num in target:
        print(train_img[num])
        img, label = random_erasing(get_img_multi(train_img[num]), get_img_multi(train_label[num]))
        '''
...
if __name__ == '__main__':
    #divide_complex('big_graph', 'dataset_naive')
    erasing('dataset_naive', 800)
